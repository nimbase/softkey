...
#   ./mock_license_server --port:18081 --keys-dir:examples/keys
...
# DEV-ONLY signing key: loaded from the seed file mint_license wrote
# (freshly generated by default), else the fixed DEMO seed below.
# NEVER SHIP either. Production signs with the real license key kept
# in an HSM / vault.
proc devOnlineKey(): Jwk =
  jwkOkpFromSeed(demoOnlineSeed(), DemoOnlineKid)

proc loadOnlineKey(keysDir: string): Jwk =
  try:
    let key = jwkOkpFromSeed(loadSeed(keysDir, OnlineSeedFile),
      DemoOnlineKid)
    stderr.writeLine("mock_license_server: loaded online key from " &
      keysDir & "/" & OnlineSeedFile)
    key
  except CatchableError:
    stderr.writeLine("mock_license_server: no seed file in " & keysDir &
      "; using fixed DEMO seed (not production-safe)")
    devOnlineKey()

proc attackerKey(): Jwk =
  ## Fresh random key per batch: the attacker never holds the real
  ## signing key, so every forged signature must fail verification.
  jwkEd25519Generate("attacker-online-key")
...
proc main() =
  ...
  let statusKey = loadOnlineKey(keysDir)
